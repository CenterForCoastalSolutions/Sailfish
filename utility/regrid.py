...
# !  Imported variable declarations.
    ...
    angle = cp.zeros((Nx, Ny), cp.Float64)
    Xinp  = cp.zeros((Nx, Ny), cp.Float64)
    Yinp  = cp.zeros((Nx, Ny), cp.Float64)


    Iout = cp.zeros2((LBi:UBi,LBj:UBj), cp.Float64)
    Jout = cp.zeros2((LBi:UBi,LBj:UBj), cp.Float64)
...
!
     ...
     &    ((Xmax-Xmin).gt.315.0_r8)).and.(spherical)) THEN
        EastLon=.TRUE.
