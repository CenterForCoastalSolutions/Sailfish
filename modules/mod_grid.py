...
class Grid:
    ...
    def __init__(self, input):
        ...
        self.angler    = cp.zeros(shape2D, dtype = cp.float32)
        self.CosAngler = cp.zeros(shape2D, dtype = cp.float32)
        self.SinAngler = cp.zeros(shape2D, dtype = cp.float32)
    

        self.f      = cp.zeros(shape2D, dtype = cp.float32)
        self.fomn   = cp.zeros(shape2D, dtype = cp.float32)
        self.grdscl = cp.zeros(shape2D, dtype = cp.float32)
        self.h      = cp.zeros(shape2D, dtype = cp.float32)
        self.latp   = cp.zeros(shape2D, dtype = cp.float32)
        self.latr   = cp.zeros(shape2D, dtype = cp.float32)
        self.latu   = cp.zeros(shape2D, dtype = cp.float32)
        self.latv   = cp.zeros(shape2D, dtype = cp.float32)
        self.lonp   = cp.zeros(shape2D, dtype = cp.float32)
        self.lonr   = cp.zeros(shape2D, dtype = cp.float32)
        self.lonu   = cp.zeros(shape2D, dtype = cp.float32)
        self.lonv   = cp.zeros(shape2D, dtype = cp.float32)
        self.Mylon  = cp.zeros(shape2D, dtype = cp.float32)
        self.omn    = cp.zeros(shape2D, dtype = cp.float32)
        self.om_p   = cp.zeros(shape2D, dtype = cp.float32)
        self.om_r   = cp.zeros(shape2D, dtype = cp.float32)
        self.om_u   = cp.zeros(shape2D, dtype = cp.float32)
        self.om_v   = cp.zeros(shape2D, dtype = cp.float32)
        self.on_p   = cp.zeros(shape2D, dtype = cp.float32)
        self.on_r   = cp.zeros(shape2D, dtype = cp.float32)
        self.on_u   = cp.zeros(shape2D, dtype = cp.float32)
        self.on_v   = cp.zeros(shape2D, dtype = cp.float32)
        self.pm     = cp.zeros(shape2D, dtype = cp.float32)
        self.pn     = cp.zeros(shape2D, dtype = cp.float32)

        # if CURVGRID && UV_ADV: TODO: Implement the if
        self.dmdη   = cp.zeros(shape2D, dtype = cp.float32)
        self.dndξ   = cp.zeros(shape2D, dtype = cp.float32)

        self.pmon_p = cp.zeros(shape2D, dtype = cp.float32)
        self.pmon_r = cp.zeros(shape2D, dtype = cp.float32)
        self.pmon_u = cp.zeros(shape2D, dtype = cp.float32)
        self.pmon_v = cp.zeros(shape2D, dtype = cp.float32)
        self.pnom_p = cp.zeros(shape2D, dtype = cp.float32)
        self.pnom_r = cp.zeros(shape2D, dtype = cp.float32)
        self.pnom_u = cp.zeros(shape2D, dtype = cp.float32)
        self.pnom_v = cp.zeros(shape2D, dtype = cp.float32)
        # self.xp     = cp.zeros(shape2D, dtype = cp.float32)
        # self.xr     = cp.zeros(shape2D, dtype = cp.float32)
        # self.xu     = cp.zeros(shape2D, dtype = cp.float32)
        # self.xv     = cp.zeros(shape2D, dtype = cp.float32)
        # self.yp     = cp.zeros(shape2D, dtype = cp.float32)
        # self.yr     = cp.zeros(shape2D, dtype = cp.float32)
        # self.yu     = cp.zeros(shape2D, dtype = cp.float32)
        # self.yv     = cp.zeros(shape2D, dtype = cp.float32)
