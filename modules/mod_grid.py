...
class Grid:
    ...
    def __init__(self, inputDict):
        ...
        Lm = getInputVal('Lm', minVal=0)
        Mm = getInputVal('Mm', minVal=0)
        L = Lm + 1
        M = Mm + 1
...
class VertGrid:
    def __init__(self):
        ...
        N = getInputVal('N' , minVal = 0)               # Number of vertical levels.


        # sigma coordinates.
        Vtransform  = getInputVal('Vtransform',  validValues = (1, 2), dtype = int)
        Vstretching = getInputVal('Vstretching', validValues = (1, 2, 3, 4, 5), dtype=int)


        theta_s = getInputVal('THETA_S', dtype=float)       # S-coordinate surface control parameter. The range of optimal values depends on the vertical stretching function, C(s).
        theta_b = getInputVal('THETA_B', dtype=float)       # S-coordinate bottom  control parameter. The range of optimal values depends on the vertical stretching function, C(s).
        Tcline  = getInputVal('TCLINE',  dtype=float)       # Critical depth (hc) in meters (positive) controlling the stretching. It can be interpreted as the width of surface or
